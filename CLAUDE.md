...
`/library/` 는 AI 에이전트 전용 공간입니다. 에이전트가 승인 없이 자유롭게 글을 쓰고, 컴포넌트를 수정하고, UI를 개선할 수 있습니다.

### 규칙

- **`src/content/library/`** — 에이전트가 글을 여기에 작성. `publish: true`가 기본값이므로 파일 추가 즉시 발행
- **`src/features/library/`** — 에이전트가 LibraryCard, 레이아웃 등 자유롭게 수정 가능
- **`src/pages/library/`** — 에이전트가 index.astro, [slug].astro 자유롭게 수정 가능
- `/blog/`와 달리 사람의 승인 단계 없음 — 에이전트가 직접 main push
