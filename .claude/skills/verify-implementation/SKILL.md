...
description: 프로젝트의 모든 verify 스킬을 병렬 실행하여 통합 검증 보고서를 생성합니다. 기능 구현 후, PR 전, 코드 리뷰 시 사용.
...
프로젝트에 등록된 모든 `verify-*` 스킬을 병렬로 실행하여 통합 검증을 수행합니다:
...
이 스킬이 병렬 실행하는 검증 스킬 목록입니다. `/manage-skills`가 스킬을 생성/삭제할 때 이 목록을 자동 업데이트합니다.

| # | 스킬 | 설명 |
|---|------|------|
| 1 | `verify-blog-post` | 발행된 블로그 포스트 전체 일괄 검증 (슬러그 중복, 이미지 프롬프트 잔여, 필수 frontmatter 누락) |
...
**실행 대상 스킬** 테이블에 나열된 모든 스킬을 **동시에** 서브에이전트로 실행합니다.

#### 2a. 병렬 서브에이전트 실행

각 verify 스킬마다 독립적인 서브에이전트를 동시에 생성합니다 (Agent tool 사용). 각 에이전트에게 다음을 전달합니다:

```
`.claude/skills/verify-<name>/SKILL.md`를 읽고 Workflow에 정의된 모든 검사를 실행하세요.
Exceptions 섹션의 패턴은 면제 처리하세요.
아래 형식으로 결과를 반환하세요:

skill: verify-<name>
total: N / passed: X / issues: Y / exempt: Z
details:
- [PASS|FAIL] <검사명> — <파일:라인> — <설명> — 수정: <방법>
```

**중요:** 스킬 수가 1개여도 서브에이전트를 사용하여 일관성을 유지합니다.

#### 2b. 결과 수집

모든 서브에이전트 완료 후 결과를 수집합니다. 각 스킬 완료 시:

```markdown
### verify-<name> 검증 완료

- 검사 항목: N개 / 통과: X개 / 이슈: Y개 / 면제: Z개
```
