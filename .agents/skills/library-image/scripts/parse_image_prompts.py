...
def selected(item, only):
    ...
    return bool(tokens & {item["slot"], item["save_as"], item["output_path"]})
...
def build_manifest(path, force=False, only=None):
    ...
    out_dir = Path("public") / "images" / "library" / slug
    ...
    for comment in comments:
        ...
        if not save_as:
            save_as = f"{auto_index:02d}_image.png"
        auto_index += 1

        slot = slot_from_name(save_as) or f"{auto_index - 1:02d}"
        aspect = comment.get("aspect_ratio") or ("4:3" if slot == "thumbnail" else "16:9")
        ...
        item = {
            ...
            "aspect_ratio": aspect,
            "session_id": comment.get("session_id") or f"library-{slug}",
            "save_as": save_as,
...
def main():
    parser = argparse.ArgumentParser(description="Parse library image prompt comments.")
    parser.add_argument("markdown")
    parser.add_argument("--only", help="Comma-separated slots or filenames, for example thumbnail,01")
