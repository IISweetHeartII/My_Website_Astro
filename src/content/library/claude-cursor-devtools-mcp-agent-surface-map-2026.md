...
featured_image_alt: "Claude Plugins, Cursor Plugins, DevTools MCP, codegraph가 코딩 에이전트 실행면으로 연결된 지도형 일러스트"
meta_title: "Claude Plugins와 Cursor Plugins, 코딩 에이전트 실행면 지도 | Library"
meta_description: "Claude Plugins, Cursor Plugins, Chrome DevTools MCP, codegraph로 보는 AI 코딩 에이전트 실행면 경쟁."
keywords:
...
og_description: "Claude Plugins, Cursor Plugins, DevTools MCP, codegraph를 한 장의 실행면 지도로 묶어 실무 선택 기준을 정리했다."
og_type: article
...
<!--
  📸 이미지 프롬프트:
  prompt: "Clean editorial tech illustration of an AI coding agent execution surface map, four connected zones labeled Claude Plugins, Cursor Plugins, Chrome DevTools MCP, and codegraph, developer workstation, browser debugger, IDE, repository graph, modern Korean tech media style, balanced blue green and graphite palette"
  aspect_ratio: "4:3"
  session_id: "library-claude-cursor-devtools-mcp-agent-surface-map-2026"
  save_as: "thumbnail.png"
-->

나는 치즈라서 새 개발 도구가 뜰 때 기능 목록보다 먼저 사람들의 반응 지도를 본다. 이번 신호가 재미있는 건 하나의 제품 뉴스가 아니라는 점이다. 2026년 5월 24일 최신 트렌딩 흐름에서 anthropics/claude-plugins-official, cursor/plugins, Chrome DevTools MCP, codegraph, opencode 같은 이름이 같은 날 눈에 들어왔다. 따로 보면 “플러그인 하나 나왔네” 정도지만, 같이 보면 더 선명하다. **코딩 에이전트 경쟁은 모델 성능표에서 개발자의 실행면을 누가 더 잘 잡느냐로 이동하고 있다.**
...
그래서 나는 이번 흐름을 “AI 코딩 도구가 많아졌다”가 아니라, **에이전트가 개발자의 작업면을 나눠 점유하기 시작했다**는 신호로 본다. Claude Plugins는 에이전트 능력을 배포하고 조합하는 면, Cursor Plugins는 IDE 안에서 확장되는 면, Chrome DevTools MCP는 브라우저 실행과 검증 면, codegraph는 저장소 이해 면을 대표한다.
...
## Claude Plugins는 능력 배포면이다

Claude Plugins의 의미는 단순히 “Claude에 기능을 추가한다”로 끝나지 않는다. 내가 보기에 핵심은 에이전트에게 줄 수 있는 업무 단위를 패키징하고, 재사용하고, 팀의 작업 규칙으로 배포할 수 있다는 데 있다.
...
다만 여기서 중요한 건 멋진 확장성보다 운영 가능성이다. 플러그인은 곧 권한과 신뢰의 묶음이다. 어떤 파일을 읽을 수 있는지, 어떤 명령을 실행할 수 있는지, 외부 API를 호출하는지, 자동 업데이트가 되는지까지 같이 봐야 한다. 그래서 Claude Plugins 흐름은 생산성 뉴스이면서 동시에 운영 거버넌스 뉴스다.

## Cursor Plugins는 IDE 안의 습관을 잡는다

Cursor Plugins 쪽은 조금 다른 위치에 있다. 이건 개발자가 이미 손을 올려둔 IDE 안에서 에이전트를 확장하는 면이다. 여기서 승부는 “에이전트가 무엇을 할 수 있나”뿐 아니라 “개발자가 흐름을 끊지 않고 그 능력을 부를 수 있나”로 갈린다.

개발자는 생각보다 보수적이다. 새 도구가 좋아 보여도 매번 창을 바꾸고, 컨텍스트를 복사하고, 결과를 다시 붙여 넣어야 하면 오래 못 간다. IDE 플러그인은 이 마찰을 줄인다. 현재 파일, 선택 영역, diagnostics, git diff, 터미널 상태, 테스트 실패를 바로 문맥으로 쓸 수 있기 때문이다.

이 표면을 잡으면 에이전트는 “채팅창에 있는 조언자”에서 “작업 중간에 손이 닿는 도구”가 된다. 작은 리팩터링, 타입 오류 수정, 테스트 보강, 문서 업데이트, 반복적인 rename 같은 작업은 IDE 안에서 바로 호출될 때 훨씬 자연스럽다.
...
<!--
  📸 이미지 프롬프트:
  prompt: "Developer IDE workspace with an AI plugin connected to current file, git diff, failing tests, terminal output, and code suggestions, clean product-style technical illustration, no brand logos, modern Korean software media aesthetic"
  aspect_ratio: "16:9"
  session_id: "library-claude-cursor-devtools-mcp-agent-surface-map-2026"
