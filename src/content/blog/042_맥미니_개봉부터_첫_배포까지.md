---
title: "[홈서버 2편] 맥미니 개봉부터 첫 배포까지 🖥️ - 홈서버의 첫걸음"
description: "맥미니 M2 언박싱, macOS 네트워크 설정, Docker 설치부터 Spring Boot 첫 실행까지의 여정"
publish: true
created_date: 2025-09-08
category: DevOps
featured_image: /images/blogs/042/042_00_thumbnail.png
featured_image_alt: "맥미니 M2 개봉 및 홈서버 첫 배포"
slug: mac-mini-setup
tags:
  - 홈서버
  - 맥미니
...
faq:
  - question: "맥미니 홈서버 초기 설정에 필요한 것은 무엇인가요?"
    answer: "HDMI 모니터, USB 키보드/마우스, 인터넷 연결이 필요합니다. 초기 macOS 설정 후에는 SSH로 원격 접속하므로 모니터를 분리할 수 있습니다."
  - question: "맥미니에서 고정 IP를 설정하는 이유는 무엇인가요?"
    answer: "SSH 접속 시 주소 변경을 방지하고, Cloudflare Tunnel 설정의 안정성을 확보하며, 방화벽 규칙 설정을 편리하게 하기 위해서입니다."
  - question: "Docker Compose로 Spring Boot와 MySQL을 어떻게 실행하나요?"
    answer: ".env 파일에 환경 변수를 설정하고, docker-compose.yml에서 서비스를 정의한 후, docker compose up --build -d 명령어로 실행합니다. healthcheck로 MySQL 준비 완료 후 Spring Boot를 시작합니다."
  - question: "맥미니 M2의 실제 전력 소비량은 얼마인가요?"
    answer: "유휴 상태 7W, Docker 실행 시 12W, 최대 부하 18W입니다. 월 전기세는 약 1,300원으로 AWS RDS 월 24,000원의 5% 수준입니다."
---

# 맥미니 개봉부터 첫 배포까지 🖥️

<!-- 📸 이미지 프롬프트: prompt: "Unboxing a Mac Mini M2, silver aluminum device on a table, clean packaging, excited hands opening the box, warm soft light, flat illustration style, modern tech aesthetic", aspect_ratio: "16:9", session_id: "blog-042", save_as: "042_00_thumbnail.png" -->
...
<!-- 📸 이미지 프롬프트: prompt: "Mac Mini M2 sitting alone in a corner of a desk without monitor or keyboard, a small green LED light indicating it is running, minimalist setup, clean home office background, flat digital illustration", aspect_ratio: "16:9", session_id: "blog-042", save_as: "042_01_headless-server.png" -->
...
<!-- 📸 이미지 프롬프트: prompt: "Terminal screen showing Spring Boot application successfully started with green SUCCESS text, Docker containers running status, health check returning UP status, dark terminal background with green and white text", aspect_ratio: "16:9", session_id: "blog-042", save_as: "042_02_first-deployment.png" -->
...
## 이전 글 / 다음 글← [이전: AWS 요금 폭탄 💸에서 맥미니 홈서버로 탈출하기](/aws-to-mac-mini) | [다음: 포트 포워딩 없이 HTTPS 열기 - Cloudflare Tunnel 🔒](/cloudflare-tunnel) →---
