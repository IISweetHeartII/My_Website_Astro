...
faq:
  - question: "Astro 블로그 성능을 최적화하는 가장 효과적인 방법은 무엇인가요?"
    answer: "이미지 최적화가 가장 중요합니다. WebP 포맷 사용, 이미지 리사이징, lazy loading 적용이 핵심이며, Astro의 Image 컴포넌트를 활용하면 이를 자동으로 처리할 수 있습니다. 추가로 코드 블록 언어 최적화와 일관된 이미지 경로 관리도 성능 향상에 도움이 됩니다."

  - question: "WebP 이미지 포맷을 사용해야 하는 이유는 무엇인가요?"
    answer: "WebP는 JPEG와 PNG의 장점을 결합한 차세대 이미지 포맷으로, 더 작은 파일 크기로 더 높은 품질을 제공합니다. 대부분의 최신 브라우저에서 지원되며, 로딩 속도 향상과 SEO 개선에 직접적인 효과가 있습니다."

  - question: "Astro의 Image 컴포넌트는 어떻게 사용하나요?"
    answer: "astro:assets에서 Image 컴포넌트를 import하고, src/assets 폴더에 이미지를 저장한 후 사용합니다. 이 컴포넌트는 자동으로 WebP 변환, srcset 생성, lazy loading을 처리해주어 수동 최적화 없이도 최상의 성능을 제공합니다."

  - question: "이미지 lazy loading이란 무엇이고 왜 중요한가요?"
    answer: "Lazy loading은 사용자가 스크롤하여 이미지가 화면에 보일 때만 로드하는 기술입니다. 페이지 초기 로딩 시 보이지 않는 이미지를 나중에 불러와 초기 로딩 속도를 크게 향상시키며, 사용자 경험과 SEO 모두에 긍정적인 영향을 미칩니다."

  - question: "Astro 블로그에서 이미지는 어디에 저장해야 하나요?"
    answer: "두 가지 방법이 있습니다. 1) src/assets 폴더에 저장하고 Image 컴포넌트로 사용하면 자동 최적화가 적용됩니다. 2) public/images 폴더에 저장하고 절대 경로(/images/...)로 사용하면 마크다운에서 직접 참조할 수 있습니다. 전자는 빌드 시 최적화가 필요한 경우, 후자는 정적 파일로 그대로 제공할 때 적합합니다."
---
...
```astro
// src/pages/post.astro (예시) import {Image} from 'astro:assets'; import myImage from
'../assets/my-image.jpg'; // src/assets 폴더에 이미지 저장
...
### Q1. Astro 블로그 성능을 최적화하는 가장 효과적인 방법은 무엇인가요?

이미지 최적화가 가장 중요합니다. WebP 포맷 사용, 이미지 리사이징, lazy loading 적용이 핵심이며, Astro의 Image 컴포넌트를 활용하면 이를 자동으로 처리할 수 있습니다. 추가로 코드 블록 언어 최적화와 일관된 이미지 경로 관리도 성능 향상에 도움이 됩니다.

### Q2. WebP 이미지 포맷을 사용해야 하는 이유는 무엇인가요?

WebP는 JPEG와 PNG의 장점을 결합한 차세대 이미지 포맷으로, 더 작은 파일 크기로 더 높은 품질을 제공합니다. 대부분의 최신 브라우저에서 지원되며, 로딩 속도 향상과 SEO 개선에 직접적인 효과가 있습니다.

### Q3. Astro의 Image 컴포넌트는 어떻게 사용하나요?

`astro:assets`에서 Image 컴포넌트를 import하고, `src/assets` 폴더에 이미지를 저장한 후 사용합니다. 이 컴포넌트는 자동으로 WebP 변환, srcset 생성, lazy loading을 처리해주어 수동 최적화 없이도 최상의 성능을 제공합니다.

### Q4. 이미지 lazy loading이란 무엇이고 왜 중요한가요?

Lazy loading은 사용자가 스크롤하여 이미지가 화면에 보일 때만 로드하는 기술입니다. 페이지 초기 로딩 시 보이지 않는 이미지를 나중에 불러와 초기 로딩 속도를 크게 향상시키며, 사용자 경험과 SEO 모두에 긍정적인 영향을 미칩니다.

### Q5. Astro 블로그에서 이미지는 어디에 저장해야 하나요?

두 가지 방법이 있습니다:

1. **src/assets 폴더**: Image 컴포넌트로 사용하면 자동 최적화 적용
2. **public/images 폴더**: 절대 경로(`/images/...`)로 마크다운에서 직접 참조

전자는 빌드 시 최적화가 필요한 경우, 후자는 정적 파일로 그대로 제공할 때 적합합니다.
