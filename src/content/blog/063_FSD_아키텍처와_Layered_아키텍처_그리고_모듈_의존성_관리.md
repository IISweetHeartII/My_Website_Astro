...
description: "FSD와 Layered 아키텍처를 비교하고, 컨벤션·ESLint·pre-commit(Husky)이 왜 더 중요한지, Omechu v2에서 pages/features 레이어를 버린 이유와 의존성 강제 방법까지 정리했다."
publish: true
meta_title: "FSD vs Layered, 그리고 모듈 의존성 관리 | 김덕환"
meta_description: "FSD와 Layered 아키텍처의 차이보다 먼저 봐야 할 컨벤션·ESLint·pre-commit(Husky). Omechu 마이그레이션에서 pages/features를 버린 이유와 의존성 강제 패턴까지 정리했다."
keywords:
...
스터디 7주차 주제는 **폴더 구조와 아키텍처**다. 그런데 이 글에서 먼저 잡아야 할 키워드는 따로 있다. **컨벤션, ESLint, pre-commit(Husky).** 내가 진짜 말하고 싶은 핵심은 폴더 이름 자체가 아니라, 소프트웨어 엔지니어링에서 이 세 가지가 구조를 실제 규율로 바꾼다는 점이다.

발표 자료를 그대로 쓰면 너무 휘발된다. 그래서 FSD, Layered, 모듈 의존성 관리, 그리고 실제 프로젝트 리팩토링 이야기를 전부 이 세 키워드로 다시 묶어서 남긴다. 폴더 구조는 겉모양이고, 컨벤션과 도구가 없으면 아키텍처는 금방 무너진다.
...
아키텍처 얘기부터 바로 들어가면 보통 순서가 뒤집힌다. 실무에서 먼저 필요한 건 아래 세 가지다.

1. **컨벤션** — "이 코드는 어디에 둔다"를 팀 공용 언어로 만드는 규칙
2. **ESLint** — 그 규칙을 말이 아니라 에러로 바꾸는 장치
3. **pre-commit(Husky)** — 규칙 위반 코드를 아예 저장소에 못 들어오게 막는 첫 관문

왜 폴더 구조 글에서 이걸 먼저 말하느냐. 폴더 구조 자체는 누구나 그릴 수 있지만, **컨벤션과 자동 검사가 없으면 그 구조는 일주일도 못 간다.** 그래서 이 글도 FSD vs Layered 비교보다, 결국 어떤 팀 규율을 택했고 어떻게 강제했는지에 더 무게를 둔다.
...
FSD 문서에는 여전히 **7개 레이어**가 적혀 있지만, `processes`는 이미 **deprecated** 표시가 붙어 있다. 그래서 실전 설명에서는 보통 `processes`를 접고 **6개 레이어**로 이해하는 편이 더 맞다. 이 글도 그 기준으로 정리한다.
...
<!--
  📸 이미지 프롬프트:
  prompt: "Top-down city map illustration showing FSD districts labeled app, processes, pages, widgets, features, entities, shared. The processes district is visibly deprecated with a red diagonal strike-through. Each district has its own buildings and small icons inside. One-way arrows between districts pointing only downward in hierarchy: app at top, shared at bottom. Roads connecting districts labeled 'public API'. Flat illustration, soft pastel colors, clean educational style"
  aspect_ratio: "16:9"
  session_id: "blog-063"
...
<!--
  📸 이미지 프롬프트:
  prompt: "Side by side comparison illustration. Left side titled 'BEFORE — FSD layers' showing a tall stack of folders: app, processes, pages, widgets, features, entities, shared. The deprecated processes folder is also shown with a red diagonal strike-through. Right side titled 'AFTER — Omechu 4 layers' showing a shorter cleaner stack: app, widgets, entities, shared. The pages and features folders on the left are shown with strikethrough, with arrows indicating they were merged into 'app' and 'widgets/entities'. Flat illustration, soft pastel colors, clean educational diagram, before/after format"
  aspect_ratio: "16:9"
  session_id: "blog-063"
...
**아키텍처는 폴더 구조가 아니라 의존성 규율이다.** 폴더만 잘 나눠놓고 import는 자유롭게 하면, 그건 FSD를 흉내낸 미로일 뿐이다. 결국 진짜 일을 하는 건 컨벤션, ESLint, 그리고 pre-commit(Husky)다.
