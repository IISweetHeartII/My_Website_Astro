...
faq:
  - question: "JWT와 Cookie의 차이점은 무엇인가요?"
    answer: "JWT는 JSON 형식의 토큰으로 서버 신원 증명에 사용되며, Cookie는 브라우저에 저장되는 작은 데이터 조각으로 사용자 상태 유지에 사용됩니다. JWT는 상태를 저장하지 않지만 Cookie는 HttpOnly와 Secure 속성으로 보안을 강화할 수 있습니다."
  - question: "Refresh Token이 필요한 이유는 무엇인가요?"
    answer: "Access Token은 보안을 위해 만료 시간이 짧습니다. Refresh Token은 유효 기간이 길어 만료된 Access Token을 갱신할 수 있어, 사용자가 자주 재로그인하지 않아도 됩니다."
  - question: "Protected Route는 어떻게 동작하나요?"
    answer: "Protected Route는 인증 상태를 확인하여 인증된 사용자만 특정 경로에 접근할 수 있도록 제어합니다. 인증되지 않은 사용자는 로그인 페이지로 리다이렉션됩니다."
  - question: "OAuth 2.0의 주요 구성 요소는 무엇인가요?"
    answer: "리소스 소유자(사용자), 클라이언트(앱), 리소스 서버(데이터 호스팅), 인가 서버(토큰 발급) 4가지가 핵심입니다. Authorization Code Grant 방식이 가장 보안이 강력합니다."
...
category: 보안
featured_image: /images/blogs/014/014_00_thumbnail.png
...
<!-- 📸 이미지 프롬프트: prompt: "A dark themed illustration of web security concepts showing a glowing padlock icon overlaid on complex code and circuit board patterns, cybersecurity digital art style, deep blue and green colors", aspect_ratio: "16:9", session_id: "blog-014", save_as: "014_00_thumbnail.png" -->
...
<!-- 📸 이미지 프롬프트: prompt: "A diagram showing JWT token structure with three parts Header, Payload and Signature separated by dots, with base64 encoded strings, technical illustration on dark background, developer documentation style", aspect_ratio: "16:9", session_id: "blog-014", save_as: "014_01_jwt-structure.png" -->
...
<!-- 📸 이미지 프롬프트: prompt: "A flow diagram illustrating OAuth 2.0 authorization code grant process with four actors: User, Client App, Authorization Server, and Resource Server, with arrows showing the token exchange flow, clean technical diagram, white background", aspect_ratio: "16:9", session_id: "blog-014", save_as: "014_02_oauth-flow.png" -->
