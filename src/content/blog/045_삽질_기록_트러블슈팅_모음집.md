---
title: "[홈서버 5편] 맥미니 홈서버 삽질 기록 🔧 - 새벽 2시의 디버깅 전쟁"
description: "Cloudflare Status 1부터 Shift 키 오작동까지, 홈서버 구축하며 겪은 모든 에러와 해결 방법 총정리"
publish: true
created_date: 2025-09-29
category: DevOps
featured_image: /images/blogs/045/045_00_thumbnail.png
featured_image_alt: "홈서버 구축 중 발생한 다양한 에러와 트러블슈팅"
...
faq:
  - question: "Docker Compose 빌드가 무한 로딩될 때 해결 방법은 무엇인가요?"
    answer: "Docker Desktop이 메모리를 가득 채웠을 가능성이 높습니다. Docker Desktop을 재시작하고 docker builder prune -a로 빌드 캐시를 삭제하세요."
  - question: "MySQL 컨테이너 연결이 실패하는 이유는 무엇인가요?"
    answer: "Spring Boot가 MySQL이 완전히 시작되기 전에 연결 시도했을 수 있습니다. docker-compose.yml에서 depends_on에 condition: service_healthy를 추가하고 MySQL에 healthcheck를 설정하세요."
  - question: "GitHub Actions에서 로컬 네트워크 서버에 SSH 접속이 안 되는 이유는 무엇인가요?"
    answer: "GitHub Actions는 클라우드(AWS)에서 실행되므로 로컬 네트워크(192.168.x.x)에 접속할 수 없습니다. Self-Hosted Runner를 사용해 맥미니에서 직접 Actions를 실행해야 합니다."
  - question: "삽질을 줄이는 가장 효과적인 방법은 무엇인가요?"
    answer: "로그부터 확인하고(docker logs, 시스템 로그), 수동 실행으로 명령어를 테스트하며, 환경 변수를 출력해 확인하고, 단계별로 검증하면서 진행하는 것이 가장 효과적입니다."
---

# 맥미니 홈서버 삽질 기록 🔧

<!-- 📸 이미지 프롬프트: prompt: "A developer surrounded by error messages and bug icons at 2am, multiple terminal windows with red error text, coffee cups, tired but determined expression, dark room with blue screen glow, flat illustration style, humorous tone", aspect_ratio: "16:9", session_id: "blog-045", save_as: "045_00_thumbnail.png" -->
...
<!-- 📸 이미지 프롬프트: prompt: "A summary infographic showing six different error categories with time spent on each, bar chart or timeline style, icons for Cloudflare, SSH, Docker, MySQL, GitHub Actions, clock symbols, flat design, clean data visualization", aspect_ratio: "16:9", session_id: "blog-045", save_as: "045_01_error-summary.png" -->
...
## 이전 글 / 다음 글← [이전: git push만으로 자동 배포 - Self-Hosted Runner 🚀](/github-actions-cicd) | [다음: 맥미니 홈서버 1개월 후기 💰](/one-month-review) →---
