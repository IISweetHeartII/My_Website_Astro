---
title: "[홈서버 4편] git push만으로 자동 배포 - Self-Hosted Runner로 CI/CD 완성하기 🚀"
...
faq:
  - question: "Self-Hosted Runner를 사용하는 이유는 무엇인가요?"
    answer: "GitHub Actions 클라우드 러너는 로컬 네트워크의 맥미니에 SSH 접속이 불가능합니다. Self-Hosted Runner는 맥미니에서 직접 실행되므로 SSH 불필요하고, 로컬 환경에서 빠르게 빌드/배포할 수 있습니다."
  - question: "Self-Hosted Runner 설치는 어떻게 하나요?"
    answer: "GitHub 저장소 Settings → Actions → Runners에서 토큰을 받고, 맥미니에 actions-runner를 다운로드한 후 config.sh로 설정하고 svc.sh install로 서비스로 등록합니다."
  - question: "runs-on: self-hosted로 변경하면 어떻게 되나요?"
    answer: "GitHub Actions가 클라우드가 아닌 맥미니에서 직접 실행됩니다. SSH 타임아웃 없이 로컬 경로(~/projects/)에 직접 접근하여 git pull, docker compose 명령을 실행할 수 있습니다."
  - question: "Self-Hosted Runner의 보안 주의사항은 무엇인가요?"
    answer: "Public 저장소에는 절대 사용하지 말고 Private 저장소에만 사용하세요. Branch Protection Rules를 설정해 main 브랜치 직접 푸시를 금지하고 PR + 리뷰를 필수로 해야 합니다."
---
...
## 이전 글 / 다음 글← [이전: 포트 포워딩 없이 HTTPS 열기 - Cloudflare Tunnel 🔒](/cloudflare-tunnel) | [다음: 삽질 기록 - 트러블슈팅 모음집 🔧](/troubleshooting) →---
