...
## 증거 1 — 에이전트 함대 "Hermes" (2026년 9월 운영 종료)

맥미니 1대에서 24시간 돌아가던 자체 운영 에이전트 시스템이다. 데모가 아니라 실제로 운영했고, 2026년 9월 맥미니 초기화와 함께 운영을 종료했다. 현재 가동 중이라고 말하지 말 것.
...
- **프로필 7개**, 그중 5개가 자체 스케줄을 보유
  ...
  - `mover` — 모멘텀 트레이딩
  - `gamjaring`
- **cron 45개** 등록 (rosie 10, kkami 16, luna 12, navi 4, mover 3), 그중 42개 활성
- **칸반 보드 기반 실행**: 카드 **383장** 완료
  - 에이전트가 카드를 **원자적으로 claim** → **격리된 워크스페이스**에서 실행 → **문서로 핸드오프**
  - 비가역적인 작업은 **사람 승인 게이트에서 멈춘다** (자율성과 안전성을 동시에)

### 이 시스템이 실제로 굴렸던 파이프라인
...
- `agentgram` org, 공개 레포 **7개** — 본체 ⭐32, `ax-score` ⭐3, `agentgram-mcp` ⭐1
- org 전체 머지된 PR **881개**, 최근 30일 **91개**
- 라이브 네트워크: 등록 에이전트 **521**, 게시물 **3,632**

### ⚠️ PR 숫자를 말할 때 반드시 지킬 것

881개 중 **789개는 김덕환의 계정으로 머지되었지만, 이는 "김덕환이 직접 손으로 작성한 PR"이 아니다.**
...
즉 숫자의 의미는 "혼자 881개를 타이핑했다"가 아니라 **"881개 규모의 변경을 자율적으로 처리하는 파이프라인을 설계하고 운영했다"** 이다. 이 구분을 흐리지 말 것.

## 증거 3 — 제품 & 수상

1. **119-ai** (2026) — 창업자/Owner. Twilio + Gemini Live API, NestJS + Python.
   여러 병원에 **동시 발신**하고 Gemini Live 음성 AI가 환자 상태를 브리핑·수락을 확인하는 **해커톤 프로토타입**(기존 순차 전화 방식을 병렬화, 실제 병원 운영 적용 아님). **청룡톤 2026 대상**.
...
- 중앙대학교 수학과(주전공) + 소프트웨어·인문융합전공(융합전공), 2026.08 졸업(2026.08.30, 유예 아님), GPA 3.86/4.5
...
- ICAN-LABs 창업 탐색 프로그램 최우수상 (2025.10)
