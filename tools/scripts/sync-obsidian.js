...
async function cleanBlogDirectory() {
  try {
    ...
    await Promise.all(
      files.map((file) => fs.unlink(path.join(ASTRO_BLOG_PATH, file)))
    );
...
function shouldPublishPost(frontmatter) {
  ...
  const publishField =
    frontmatter.publish ?? frontmatter.published ?? frontmatter.draft;
...
async function processMarkdownFile(filePath) {
  try {
    ...
    if (shouldPublishPost(frontmatter)) {
      ...
      console.log(
        `✅ Copied ${path.relative(PROJECT_ROOT, filePath)} to blog directory`
      );
    } else {
      console.log(
        `ℹ️ Skipped ${path.relative(
          PROJECT_ROOT,
          filePath
